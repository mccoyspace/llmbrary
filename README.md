...
├── wiki_src/                 # Wiki page sources (editable markdown)
│   └── *.md                  # Essays with [[Title]] catalog citations
├── wiki/                     # Compiled thematic pages (generated from wiki_src/)
│   ├── INDEX.md              # Table of contents for all wiki pages
│   └── *.md                  # ~30 thematic essays tracing intellectual threads
├── scripts/
...
| `in_conversation_with` | string[] | Related titles in the collection (max 7, cross-media) |
...
Essays are **authored as plain markdown in `wiki_src/`** — no code required to write or edit one. Citations to catalog holdings use `[[Title]]` markers:

```markdown
Guy Debord's [[La Société du Spectacle]] anchors one of the collection's
deepest intellectual threads...
```

`scripts/compile_wiki.py` resolves every marker against `catalog.json` at build time, rendering it with live author/year metadata (`*La Société du Spectacle* (Guy Debord, 1967)`). This keeps synthesis (editable prose) separate from rendering (deterministic output) — and makes hallucinated citations impossible: a `[[Title]]` that doesn't exist in the catalog is reported and left visibly unresolved rather than silently fabricated.

```bash
# Compile wiki_src/ into wiki/ + INDEX.md
python3 scripts/compile_wiki.py

# Verify all citations resolve without writing anything
python3 scripts/compile_wiki.py --check
```

`scripts/regenerate.py` compiles the wiki alongside the context tiers. The older `generate_wiki.py` remains as a compatibility wrapper that delegates to the compiler.

Wiki pages are useful for LLM context when you want to understand the intellectual landscape of a topic area rather than individual entries. See `wiki/INDEX.md` for the full table of contents.
...
The script writes the source page to `wiki_src/` (so it survives every regeneration), then recompiles `wiki/` and `INDEX.md` with live catalog citations. Use `[[Title]]` markers in the content to cite holdings; the compiler verifies each one against `catalog.json`. It's stdlib-only with no external dependencies.
...
- Include `in_conversation_with` references to existing titles where thematic connections exist (max 7, cross-media allowed)
